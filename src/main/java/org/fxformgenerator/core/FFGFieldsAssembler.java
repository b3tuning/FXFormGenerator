...
public class FFGFieldsAssembler {
	...
	protected VBox assembly(List<FFGInputGroup> inputGroups) {
		return switch (this.formLayout) {
			case FFGLayout.MULTIPLE_COLUMNS -> this.assemblyMultipleColumns(inputGroups);
			case FFGLayout.MULTIPLE_FULLWIDTHROWS_COLUMNS -> this.assemblyMultipleColumnsRowsFilled(inputGroups);
			default -> this.assemblySingleColumn(inputGroups);
		};
	}
	...
	private VBox assemblyMultipleColumnsRowsFilled(List<FFGInputGroup> inputGroups) {
		...
		for (int i = 0; i < inputGroups.size(); i += numColumns) {
			...
			if (i + (numColumns - 1) < inputGroups.size()) {
				...
				double rowWidth =
						(rowInputGroups.get(0).getMinMaxEditorWidth() * numColumns) + (10 * (numColumns - 1));
				...
				rowInputGroups.forEach(inputGroup -> inputGroup.setMinMaxEditorWidth(adaptedColumnWidth));
				formRow = this.putInRow(inputGroups.subList(i, inputGroups.size()));
			}
	...
	private HBox putInRow(List<FFGInputGroup> inputGroups) {
		...
		inputGroups.forEach(inputGroup -> hBox.getChildren().add(inputGroup.buildAsVBox()));
