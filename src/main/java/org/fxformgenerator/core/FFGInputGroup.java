	...
	public Node constructEditor() {
		...
		if (customEditor != null) {
			...
			fieldCB.getSelectionModel()
			       ...
			       .addListener((obs, oldV, newV) -> this.updateFieldValue(newV));
			...
			fieldTF.textProperty().addListener((obs, oldV, newV) -> this.updateFieldValue(newV));
			...
			fieldCB.selectedProperty().addListener((observable, oldValue, newValue) -> this.updateFieldValue(newValue));
			...
			Spinner<Double> fieldSP = new Spinner<>(new SpinnerValueFactory.DoubleSpinnerValueFactory(
					Float.MIN_VALUE,
					Float.MAX_VALUE,
			...
			fieldDP.valueProperty().addListener((obs, oldV, newV) -> updateFieldValue(newV));
